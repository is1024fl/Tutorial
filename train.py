...
# Tensorboad
writer = SummaryWriter(PJ('./results/logs', exp_name))
# Saving config file
...
iteration, last_epoch = (iteration, last_epoch) if resume else (0, 0)

# Learning rate decay scheduler
scheduler = lr_scheduler.MultiStepLR(
    optimizer, config["step_size"], config["gamma"], last_epoch=last_epoch - 1)
...
for epoch in range(last_epoch, config["max_epoch"]):
    ...
    # Saving model
    torch.save(model.state_dict(), PJ(save_root, f"{epoch+1:04d}_{iteration:08d}.pt"))
    ...
    print(f"[{epoch + 1}] Accuracy: {accuracy:0.4f}\n")
    writer.add_scalar("Evaluate/accuracy_val", accuracy, epoch + 1)
